...
def load_e():
    ...
    with open("Icmp_data.txt", "r") as icmp_file:
        for line in icmp_file:
            parts = line.strip().split(":")
            if len(parts) == 2:
                icmp, protocol = int(parts[0]), parts[1]
                ICMP[icmp] = protocol
...
def uloha_4h_load(packet1):  # nacitanie ulohy 4h
    ...
    if num_dec > 1500:
        if num_dec == 2048:
            ...
            if  ip_protocol == "ICMP":
                source, dest = extract_ipv4_ip(packet1)
            ...
            if num_dec > 1500:
                if num_dec == 2048:
                    ...
                    if ip_protocol == "ICMP":
                        ...
                        counter_reply += 1
                        print("Ethernet II")
                        dest_mac, src_mac = extract_mac(z)
                        print("Zdrojová MAC adresa: ", end="")
                        print(src_mac)
                        print("Cielova MAC adresa: ", end="")
                        print(dest_mac)
                        print_protocol_ether_type(num_dec)
                        source, dest = extract_ipv4_ip(z)
